...
def generate_image(model:DFUNet, scheduler:DDPMScheduler) -> torch.Tensor:
    ...
    sample_image = Image.open('D:/0-nebula/dataset/Havard/MyDatasets/CT-MRI/test/CT/2004.png')
    sample_array = np.array(sample_image)
    sample_tensor = torch.from_numpy(sample_array).unsqueeze(0).float()
    # sample_tensor = (sample_tensor / 127.5) - 1
    noise_sample = sample_tensor.to(device)
    images = noise_sample
    # 根据时间步逐步去噪
    for t in scheduler.inf_timesteps: 
        with torch.no_grad():
