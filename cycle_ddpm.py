...
class CycleDDPM(nn.Module):
    ...
    def compute_loss(self, x_A:Tensor, x_B:Tensor,lambda_cycle:float=1.0) -> dict[str,Tensor]:
        ...
        # 生成mri图像
        x_noisy_mri, noise_mri = self.forward_diffusion_to_mri(x_B, timesteps) #生成mri图像的添加噪声操作
        predicted_noise_mri = self.predict_noise_mri(x_noisy_mri, timesteps,condition=x_A) # 输入的x_A为ct，作为训练条件

        # 生成ct图像
        x_noisy_ct, noise_ct = self.forward_diffusion_to_ct(x_A, timesteps) #生成ct图像的添加噪声操作
        predicted_noise_ct = self.predict_noise_ct(x_noisy_ct, timesteps,condition=x_B) # 输入的x_B为mri，作为训练条件
