...
class CycleDDPM(nn.Module):
    ...
    def generate_ct(self, mri_image: Tensor) -> Tensor:
        ...
        x = torch.randn_like(mri_image).to(device=mri_image.device)  # 初始随机噪声
    ...
    def generate_mri(self, ct_image: Tensor) -> Tensor:
        ...
        x = torch.randn_like(ct_image).to(device=ct_image.device)  # 初始随机噪声
    ...
    def generate_ct_with_grad(self, mri_image,is_inference=False):
        """
        ...
        for t in timesteps:
            ...
            x = self.scheduler.step(predicted_noise, t, x,is_inference=is_inference) # type:ignore
        return x
    def generate_mri_with_grad(self, ct_image,is_inference=False):
